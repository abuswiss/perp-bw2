...
import { DocumentDraftingAgentInputSchema, DocumentDraftingAgentOutputSchema } from './schemas';

export class DocumentDraftingAgent extends BaseAgent {
  id = 'document-drafting-agent';
  type = 'writing' as const;
  name = 'Document Drafting Agent';
  description = 'Generate comprehensive legal documents including memoranda, briefs, contracts, and other professional legal writing';
  ...
  protected getInputSchema() {
    return DocumentDraftingAgentInputSchema;
  }

  protected getOutputSchema() {
    return DocumentDraftingAgentOutputSchema;
  }
  ...
  private async initializeLLM() {
    try {
      ...
      console.error('Failed to initialize LLM for DocumentDraftingAgent:', error);
    }
  }
  ...
  async execute(input: AgentInput): Promise<AgentOutput> {
    ...
    try {
      ...
      const lengthPreference = this.analyzeDocumentLengthIntent(input.query, input.parameters);
      ...
      const sourceMaterial = await this.gatherSourceMaterial(input, researchContext);

      // Step 3: Create document outline using AI
      await this.logExecution(taskId, 'Creating document outline', 40);
      const outline = await this.createOutlineWithAI(documentType, sourceMaterial, input.query, lengthPreference);

      // Step 4: Generate document content using AI
      await this.logExecution(taskId, 'Generating document content', 70);
      const content = await this.generateContentWithAI(outline, sourceMaterial, template, lengthPreference);
      ...
      await this.logExecution(taskId, 'Saving generated document', 95);
      let documentId: string | null = null;
      
      try {
        documentId = await this.saveGeneratedDocument(
          input.matterId,
          formattedDocument,
          documentType,
          {
            title: outline.title,
            summary: `Generated ${documentType} with ${this.countWords(formattedDocument)} words`,
            ...
            usedCachedResearch: researchContext?.hasResearch || false,
            cachedResearchCount: researchContext?.researchCount || 0
          }
        );
        console.log(`📝 Document saved with ID: ${documentId}`);
      } catch (saveError) {
        console.error('Failed to save document, but continuing:', saveError);
      ...
      const output: AgentOutput = {
        ...
        result: {
          ...
          sourceMaterial,
          documentId: documentId, // Include saved document ID
          metadata: {
            wordCount: this.countWords(formattedDocument),
            pageCount: Math.ceil(this.countWords(formattedDocument) / 250),
            ...
            usedCachedResearch: researchContext?.hasResearch || false,
            cachedResearchCount: researchContext?.researchCount || 0,
            savedToDatabase: documentId !== null
          }
        },
        citations,
        metadata: {
      ...
      // Cache the generated brief for potential reuse
      await this.logExecution(taskId, 'Caching generated brief for future use', 99);
      const cacheableResult = {
        type: 'brief_document',
        title: `Brief: ${documentType} - ${input.query.substring(0, 80)}${input.query.length > 80 ? '...' : ''}`,
        summary: `Generated ${documentType} with ${this.countWords(formattedDocument)} words`,
        data: {
          documentType,
          content: formattedDocument,
        ...
        expirationHours: 72 // Briefs stay fresh for 3 days
      };
  ...
  private analyzeDocumentLengthIntent(query: string, parameters?: Record<string, any>): {
    lengthPreference: 'brief' | 'standard' | 'comprehensive' | 'extensive';
    targetMultiplier: number;
  } {
    const lowerQuery = query.toLowerCase();
    
    // Check for explicit length indicators
    if (lowerQuery.includes('brief') || lowerQuery.includes('short') || lowerQuery.includes('concise') || 
        lowerQuery.includes('quick') || lowerQuery.includes('summary') || lowerQuery.includes('one page')) {
      return { lengthPreference: 'brief', targetMultiplier: 0.5 };
    }
    
    if (lowerQuery.includes('comprehensive') || lowerQuery.includes('extensive') || 
        lowerQuery.includes('detailed') || lowerQuery.includes('thorough') || 
        lowerQuery.includes('in-depth') || lowerQuery.includes('exhaustive')) {
      return { lengthPreference: 'extensive', targetMultiplier: 2.0 };
    }
    
    if (lowerQuery.includes('very detailed') || lowerQuery.includes('extremely comprehensive') ||
        lowerQuery.includes('go above and beyond') || parameters?.extensive === true) {
      return { lengthPreference: 'comprehensive', targetMultiplier: 1.5 };
    }
    
    // Default to standard length - appropriate for most real-world use cases
    return { lengthPreference: 'standard', targetMultiplier: 1.0 };
  }
...
Return only the document type identifier.`);
  ...
  private async gatherSourceMaterial(input: AgentInput, researchContext?: any): Promise<any> {
    const sources = {
      matterDocuments: [] as any[],
      researchResults: [] as any[],
      caseNotes: [] as any[],
      statutes: [] as any[],
      cachedResearch: undefined as any
    };
    ...
    try {
      ...
      // Incorporate cached research results if available
      if (researchContext?.hasResearch) {
        sources.cachedResearch = researchContext.researchSummary;
        console.log(`📚 Incorporated ${researchContext.researchCount} cached research results into source material`);
      }
  ...
  private async createOutlineWithAI(
    ...
    sourceMaterial: any,
    query: string,
    lengthPreference?: { lengthPreference: string; targetMultiplier: number }
  ): Promise<any> {
    if (!this.llm) {
      return this.createOutlineBasic(documentType, sourceMaterial, query, lengthPreference);
    }

    const lengthPref = lengthPreference || { lengthPreference: 'standard', targetMultiplier: 1.0 };
    
    // Adjust base word counts based on length preference
    const baseWordCounts = {
      brief: { minPerSection: 150, minTotal: 800 },
      standard: { minPerSection: 300, minTotal: 1500 },
      comprehensive: { minPerSection: 500, minTotal: 2500 },
      extensive: { minPerSection: 800, minTotal: 4000 }
    };
    
    const wordTargets = baseWordCounts[lengthPref.lengthPreference as keyof typeof baseWordCounts] || baseWordCounts.standard;
    
    const lengthGuidance = {
      brief: 'Create a CONCISE outline focusing on key points only. Be direct and to the point.',
      standard: 'Create a PROFESSIONAL outline with appropriate detail for standard legal practice.',
      comprehensive: 'Create a COMPREHENSIVE outline that thoroughly addresses all aspects.',
      extensive: 'Create an EXTENSIVE and DETAILED outline that goes above and beyond, exploring every nuance.'
    };
    
    const outlinePrompt = PromptTemplate.fromTemplate(`
${lengthGuidance[lengthPref.lengthPreference as keyof typeof lengthGuidance] || lengthGuidance.standard}
...
{{
  ...
  "sections": [
    {{
      "title": "Section name",
      "description": "Description of what this section should cover",
      "keyPoints": ["list of key points to address"],
      "wordTarget": estimated_word_count
    }}
  ],
  "totalWordTarget": total_estimated_words,
  "approach": "Description of the analytical approach"
}}

Word count guidance:
- Minimum {minPerSection} words per major section
- Target total of {minTotal} words
- Adjust section count and depth based on the length preference`);
    ...
    try {
      ...
      const result = await chain.invoke({ 
        ...
        documentType, 
        sourceSummary,
        lengthPreference: lengthPref.lengthPreference,
        minPerSection: wordTargets.minPerSection,
        minTotal: wordTargets.minTotal
      });
  ...
  private createOutlineBasic(
    ...
    return {
      ...
      sections: template.sections.map((section: string) => ({
        ...
        keyPoints: [],
        wordTarget: this.getSectionWordCount(section, documentType, lengthPreference?.targetMultiplier || 1.0)
      })),
      totalWordTarget: Math.round(template.sections.reduce((sum: number, section: string) => 
        sum + this.getSectionWordCount(section, documentType, lengthPreference?.targetMultiplier || 1.0), 0
      )),
  ...
  private getSectionWordCount(section: string, documentType: string, lengthMultiplier: number = 1.0): number {
    const baseWordCounts = {
      'Header': 50,
      'Question Presented': 150,
      'Brief Answer': 250,
      ...
      'Table of Authorities': 200,
      'Statement of Issues': 300,
      'Key Terms Analysis': 500,
      'Preliminary Statement': 200,
      'General Objections': 300,
      'Responses to Requests': 600,
      'Privilege Log': 250,
      'Certificate of Service': 100,
      'Limitations and Assumptions': 200
    };

    const baseCount = baseWordCounts[section as keyof typeof baseWordCounts] || 300;
    return Math.round(baseCount * lengthMultiplier);
  }
  ...
  private async generateSectionContentWithAI(
    ...
    const contentGuidance = {
      brief: `You are a legal writer creating a CONCISE section. Focus on key points only, be direct and efficient.`,
      standard: `You are a professional legal writer. Create appropriate content with standard legal detail and analysis.`,
      comprehensive: `You are an expert legal writer creating a COMPREHENSIVE section. Provide thorough analysis and detailed coverage.`,
      extensive: `You are an expert legal writer creating an EXTENSIVE section. The client expects exhaustive, in-depth analysis that explores every angle.`
    };
...
- Aim for clarity over comprehensiveness`,
      standard: `
- Cover all key points with appropriate detail
- Provide standard legal analysis
- Include relevant citations and authorities
- Balance thoroughness with readability`,
      comprehensive: `
- Provide detailed analysis of each point
- Include multiple perspectives and considerations
- Extensive use of sources and authorities
- Thorough examination of implications`,
      extensive: `
- Exhaustive analysis exploring all angles
- Multiple paragraphs per key point
- Extensive legal reasoning and progression
- Comprehensive examination of counterarguments
- Detailed practical applications and scenarios`
    };
    
    const sectionPrompt = PromptTemplate.fromTemplate(`
${contentGuidance[lengthPref.lengthPreference as keyof typeof contentGuidance] || contentGuidance.standard}
...
CONTENT REQUIREMENTS:${depthRequirements[lengthPref.lengthPreference as keyof typeof depthRequirements] || depthRequirements.standard}

GUIDELINES:
- Match the depth of analysis to the length preference
- Use available source material appropriately
- For missing information, use placeholders like [SPECIFIC DETAIL TO BE INSERTED]
- Maintain professional legal writing style
- Include citation placeholders where sources would be referenced
...
Do not include section headers in your response - just the content.`);
  ...
  private formatSourceMaterialForPrompt(sourceMaterial: any): string {
    ...
    if (sourceMaterial.cachedResearch?.length > 0) {
      formatted.push('\nCached Research Results:');
      sourceMaterial.cachedResearch.forEach((research: any, i: number) => {
        formatted.push(`${i + 1}. ${research.title} (Relevance: ${(research.relevanceScore * 100).toFixed(0)}%)`);
        if (research.summary) {
          formatted.push(`   Summary: ${research.summary.substring(0, 150)}...`);
        }
      });
    }
    ...
    return formatted.length > 0 ? formatted.join('\n') : 'No specific source material available. Please use placeholder text for facts and legal details.';
  }
...
`;
  ...
  private detectFollowUpRequest(query: string): boolean {
    const followUpKeywords = [
      'make it', 'make this', 'make the', 'revise', 'modify', 'change', 'update', 'edit',
      'shorter', 'longer', 'more detailed', 'more concise', 'expand', 'summarize',
      'add to', 'remove from', 'improve', 'rewrite', 'rework', 'adjust'
    ];
